...
export default function AtendimentoChannelsPage() {
  ...
  const loadInstances = useCallback(async () => {
    ...
    try {
      ...
      console.error(err)
      setData(null)
      showMsg('error', 'Não foi possível carregar os canais do atendimento.')
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  const handleConnect = async (suggestedLabel?: string) => {
    ...
    const labelInput = window.prompt('Nome do canal no CRM e no Chatwoot:', defaultLabel)
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-slate-600">
                  Gerencie os números de WhatsApp conectados à operação do atendimento.
                </p>
              </div>
            </div>
          </div>
      ...
      {planUsage && !planUsage.canAddMore && !planUsage.isUnlimited && (
        <div className="rounded-2xl border border-yellow-200 bg-yellow-50 px-4 py-4 shadow-sm">
          ...
          <p className="mt-1 text-sm text-yellow-700">
            Desconecte um canal atual ou faça upgrade para liberar mais instâncias.
          </p>
        </div>
      )}
